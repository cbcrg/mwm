...
real_t_stat <- f_t_stat (new_coord_real_lab, gen_1 = "TS", gen_2 = "TSEEEGCG", acq_day=5)
real_t_stat
...
significance_perm <- function (tbl_perm, gr1="TS", gr2="TSEEEGCG", n_perm=10001, a_day=5)  {
  real_t_stat <- f_t_stat (new_coord_real_lab, gen_1 = gr1, gen_2 = gr2, acq_day=a_day)
...
#   v <- c (as.character(gentreat_pairs [row,1]), as.character(gentreat_pairs [row,2]), paste (gentreat_pairs [row,1], gentreat_pairs [row,2], sep="_"), f_t_stat (new_coord, gentreat_pairs [row,1], gentreat_pairs [row,2]))
  v <- c (as.character(gentreat_pairs [row,1]), as.character(gentreat_pairs [row,2]), paste (gentreat_pairs [row,1], gentreat_pairs [row,2], sep="_"), significance_perm (tbl_1111, gentreat_pairs [row,1], gentreat_pairs [row,2], a_day=5)) 
  
  df.t_stats <- rbind (df.t_stats, v)
  colnames (df.t_stats) <- c ("gr1", "gr2", "comp", "t_stat")
...
real_t_stat <- f_t_stat (new_coord_real_lab, gr1, gr2, acq_day=5)
f_t_stat (new_coord_real_lab, "TSEEEGCG", "TSEGCG", acq_day=5)
"TSEEEGCG", "TSEGCG"
gr1<-"TSEEEGCG"
gr2<-"TSEGCG"
tbl_perm<-tbl_1111
...
t_perm_gr1_gr2 <- t_perm_gr1_gr2 [order(t_perm_gr1_gr2)]
(length (t_perm_gr1_gr2) - length(t_perm_gr1_gr2 [t_perm_gr1_gr2 < real_t_stat])) / length (t_perm_gr1_gr2)
