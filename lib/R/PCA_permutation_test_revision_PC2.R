...
# paste (tbl_ind[,1], gsub ("Day ", "", tbl_median[,2]), sep="")

# gentreat

# new_coord_real_lab$gen_day <- row.names(new_coord_real_lab)

# new_coord_real_lab$day <- as.factor(as.numeric (gsub(".*([0-9]+)$", "\\1", new_coord_real_lab$gen_day)))
# new_coord_real_lab$genotype <- as.factor(gsub("([A-Z]+).*$", "\\1", new_coord_real_lab$gen_day))
# new_coord_real_lab$genotype <- factor(new_coord_real_lab$genotype , levels=c("WT", "TS", "WTEE", "TSEE", "WTEGCG", "TSEGCG", "WTEEEGCG", "TSEEEGCG"), 
...
#   group1 <- subset (new_coord_real_lab, genotype == gen_1 & day==acq_day)
#   group2 <- subset (new_coord_real_lab, genotype == gen_2 & day==acq_day)
#   t_stat = t.test (group1$Dim.1, group2$Dim.1)$statistic  
#   return (t_stat)
# }
...
tbl_1111_day5 <- read.table ("/Users/jespinosa/20150515_PCA_old_frotiersPaper/data/PCA_t_statistic_1111_day5.csv", sep="\t", dec=".", header=F, stringsAsFactors=F)

df.sign_threshold.day5 <- sign_threshold (tbl_1111_day5, day=5)
df.sign_threshold.day1 <- sign_threshold (tbl_1111_day1, day=1)

write.table(df.sign_threshold.day5, file = paste(home, "/20150515_PCA_old_frotiersPaper/tbl", "/tbl_sign_thresh_perm_5_revision.csv", sep=""), 
            sep="\t", row.names=FALSE, dec = ",", col.names=TRUE)

write.table(df.sign_threshold.day1, file = paste(home, "/20150515_PCA_old_frotiersPaper/tbl", "/tbl_sign_thresh_perm_1_revision.csv", sep=""), 
            sep="\t", row.names=FALSE, dec = ",", col.names=TRUE)
