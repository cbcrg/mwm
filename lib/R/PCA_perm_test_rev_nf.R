#############################################################################
### Jose A Espinosa. NPMMD/CB-CRG Group. Sept 2015                        ###
#############################################################################
### MWM Paper Silvina frontiers                                           ###
### t-test statistic calculation for two PCA group comparison by doing    ### 
### several permutations of the original animal genotypes REVERSAL        ###
### Cluster runs                                                          ###
#############################################################################
...
wd <- getwd()
write.table(result, file = paste(wd, "/tbl_t_stat_rev3.csv", sep=""), sep="\t", row.names=FALSE, col.names=FALSE)
write.table(result_1, file = paste(wd, "/tbl_t_stat_rev1.csv", sep=""), sep="\t", row.names=FALSE, col.names=FALSE)
