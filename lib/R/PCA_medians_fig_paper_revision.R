...
ResMMALL <- prcomp(tbl_median[,c(3:9)],scale=TRUE)
summary(ResMMALL)
res_MMALL_PCAfunction = PCA(tbl_median[,c(3:9)], scale.unit=TRUE) 
summary (res_MMALL_PCAfunction)

pca_2<- PCA(M.med,scale.unit = TRUE)
pca_3<- prcomp (M.med, scale=TRUE)
summary(pca_2)
summary(pca_3)
...
######################################
old_ma2=spss.get("/Users/jespinosa/20150515_PCA_old_frotiersPaper/data/Ts65Dn OLD ACQ1_ACQ5_SUBCONJ.sav")
old_ma2
tail(old_ma2)

tail(ma2)
# M=as.matrix(ma2[ma2$day=="Day 5",c(4,5,7,8,9,10,11,12)])
# M=as.matrix(ma2[ma2$day=="Day 5",c(6,7,8,9,10)])
# head(ma2)
# M=as.matrix(ma2[ma2$day=="Day 1", c(4,5,6,7,8,9,10)])

# old_ma2=as.matrix(old_ma2[old_ma2$day=="Day 5",c(4,5,7,8,9,10,11,12)])
######################################
# head (old_ma2)
# length(ma2[,1])
# rownames(ma2) <- c(1:455)
# class(ma2)
# ma2$ID
head(ma2)
M=as.matrix(ma2[ma2$day=="Day 5", c(4,5,6,7,8,9,10)])
treat=as.matrix(ma2[ma2$day=="Day 5",2])
M4=as.matrix(ma2[ma2$day=="Day 4", c(4,5,6,7,8,9,10)])
treat4=as.matrix(ma2[ma2$day=="Day 4",2])
M3=as.matrix(ma2[ma2$day=="Day 3", c(4,5,6,7,8,9,10)])
treat3=as.matrix(ma2[ma2$day=="Day 3",2])
M2=as.matrix(ma2[ma2$day=="Day 2",c(4,5,6,7,8,9,10)])
treat2=as.matrix(ma2[ma2$day=="Day 2",2])
M1=as.matrix(ma2[ma2$day=="Day 1",c(4,5,6,7,8,9,10)])
...
head(M)
colnames(MMALL)=c("dist","gallindex","latency","speed","percentne","wishaw","percentperi")
...
res_MMALL_PCAfunction = PCA(MMALL, scale.unit=TRUE) 
summary_res_PCAfunction <-summary(res_MMALL_PCAfunction) 
summary_res <- summary(ResMMALL)

# Variance of PC1 and PC2
var_PC1 <- round (summary_res$importance [2,1] * 100, 0)
var_PC2 <- round (summary_res$importance [2,2] * 100, 0)
...
pca_medians_acq <- ggplot(pca2plot, aes(x=PC1, y=-PC2, colour=gentreat )) + 
  #                           geom_path (size = 1,show_guide = T) + 
  geom_path (size = 1,show_guide = F) + 
#   geom_point (size = 1,show_guide = F) + 
  scale_color_manual(values=c("red", "darkgreen", "blue", "lightblue", 
                              "magenta", "orange", "gray", "black")) +
  #                           geom_text (aes (label=days), vjust=-0.5, hjust=1, size=4, show_guide = T)+
  geom_text (aes (label=days), vjust=-0.5, hjust=1, size=4, show_guide = F)+
...
# I build a matrix with the individuals M.ind
# and a second matrix with the medians of group

acq=c("Day 1","Day 2","Day 3","Day 4","Day 5")
var=c(4,5,6,7,8,9,10)
M.ind=as.matrix(ma2[ma2$day%in%acq,var])
iddaytreat=as.matrix(ma2[ma2$day%in%acq,1:3])
length(iddaytreat_rev[,1])
head(iddaytreat)
head(iddaytreat[,1])
class (iddaytreat)
# colnames(iddaytreat)
rownames(iddaytreat)=c(1:n_animals)
tgt=sort(unique(iddaytreat[,2]))


# Medians of the group 
M.med=matrix(0,40,7)
rnames=c(1:40)
...
length(jm[,1])
str(jm)
# Deberia haber mas individuos en este caso
library(FactoMineR)
res = PCA(jm, scale.unit=TRUE, ind.sup=c(41:n_animals)) 
res$eig
plot(res,choix="var")
...
plot(res$var$coord[,1],res$var$coord[,2],asp=1,ylim=c(-1.1,1.1),type="n",main="Mapped variables",xlab="PC1 (72% of variance)",ylab="PC2 (14% of variance)")
...
p_cloud_wt_acq1 <- ggplot(PC1_WT_acq1, aes(PC1, PC2, color=genotype_tt)) + 
  stat_density2d(aes(fill=factor(genotype_tt), alpha = ..level..), 
                 #                  geom="polygon", color=NA, n=100, h=4, bins=6, show_guide = FALSE) +
                 geom="polygon", color=NA, h=5, n=100, bins=6, show_guide = FALSE) +
  #   scale_alpha(range = c(0.00, 1)) +
...
PC1_WT_acq5 <- subset(PC1_WT, day=="5", c("V1", "V2", "day","genotype", "id"))

colnames (PC1_WT_acq5) <- c("PC1","PC2", "day", "genotype_tt", "id")
# p_cloud_WT_acq5 <- ggplot(PC1_WT_acq5, aes(PC1, PC2, color=genotype_tt, label=id)) + 
p_cloud_wt_acq5 <- ggplot(PC1_WT_acq5, aes(PC1, PC2, color=genotype_tt)) + 
  stat_density2d(aes(fill=factor(genotype_tt), alpha = ..level..), 
                 geom="polygon", color=NA, n=100, h=5, bins=6, show_guide = FALSE) + 
  #   geom_smooth(se=F, method='lm', show_guide = FALSE) + 
  geom_point(show_guide = FALSE) + 
